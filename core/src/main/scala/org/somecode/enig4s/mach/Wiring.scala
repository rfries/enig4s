...
object Wiring:
  ...
  def apply(points: Seq[Glyph]): Either[String, Wiring] = points match
    case v if v.isEmpty =>
  ...
  def apply(mapping: String, symbols: SymbolMap): Either[String, Wiring] =
    symbols.stringToGlyphs(mapping).flatMap(Wiring.apply)

  def passthrough(size: Int): Either[String, Wiring] = apply(Range(0, size).map(Glyph.unsafe).to(ArraySeq))

  given Eq[Wiring] = Eq.fromUniversalEquals[Wiring]
