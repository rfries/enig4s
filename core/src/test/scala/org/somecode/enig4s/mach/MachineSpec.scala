...
final class MachineSpec extends AnyWordSpec with should.Matchers:
  ...
  "Machine" should {
    "encrypt a string with basic wheel settings" in {
      ...
      val reflector = Reflector(busSize, Wirings.B).require
      val machState = machineState(Vector('A' -> 'A', 'A' -> 'A', 'A' -> 'A'))
      val in = "AAAAA"

      Machine(busSize, SymbolMap.AZ, Wirings.ETW, wheels, reflector, EnigmaPlugBoard.empty(reflector.busSize)) match
    ...
    "encrypt a valid string with basic wheel settings and plugs" in {
      ...
      Machine(busSize, SymbolMap.AZ, Wirings.ETW, wheels, reflector, plugboard) match
